...
device.setInfo                 // Выставить параметры устойства.
  params:
      timeZoneOffset <number>  // Сдвиг тайм зоны в минутах (Москва = -180)

device.setFcmToken             // Установить FCM токен.
  params:
      fcmToken <string>        // FCM токен

device.resetFcmToken           // Сбросить текущий FCM токен.
...
-   `GLS_NOTIFICATION_CONNECT` _(обязательно)_ - адрес подключения к микросервису нотификаций.

-   `GLS_NOTIFICATION_SENDER_CONNECT` _(обязательно)_ - адрес подключения к микросервису нотификаций.
...
-   `GLS_GEOIP_CONNECT` _(обязательно)_ - адрес подключения к микросервису GeoIP.

-   `GLS_CONFIG_CONNECT` _(обязательно)_ - адрес подключения к микросервису конфигурации клиентов.
